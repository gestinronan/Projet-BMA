/******************
* filename: CreateTerMetroFootRelation.js
* date: 11/02/13
...
// This file create the relations between each Metro Stops and TerStops by foot
...
// Local variable
var terData = new Array();

/**
* Due to the big amount of data, we first get the Ter data
* And then we stream the Ter stops data, Calcul the time and distance between the streamed data and the data from 
...
*/
function streamMetroTableData(){

  console.log('StreamBikedata');
  var i = 0;
  // Query the database
...
*/
function compareMetroTer(metroStop){

  console.log("Compare the busStop :: " + metroStop.MetroStop_name);
  // Parse the busStop list
  for(i=0; i<terData.length; i++){

    // Check if the bus stop is not the same as the one in parameter
    if(terData[i].Stop_id != metroStop.MetroStop_id){
    
      // Get the approximative distance
      getApproximativeFootDistance(terData[i], metroStop);
    }
  }
}
