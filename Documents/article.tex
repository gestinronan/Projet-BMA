...
\section{Approche (Comment le porbl\`eme a \'et\'e trait\'e}
	...
	les Personne qui cherchent a utiliser les nouveau moyen de transport, ont besoin d'un outils a la fois, rapide, facile d'utilisation et surtout fiable.
	Dans cette démarche, nous avons d/'ecider de proposer un application avec un design  simple, une carte et une option pour poser les question sur les trajet que l'on peut glisser dessu. Pour \'évité un trop grand nombre d'appel vers les serveur des différent partenaire nous avons travailler sur un serveur personnel Nodejs, qui permet de travaillier a la fois sur les donn\'es brute et sur les infromations de trajet. Pour c'est dernier nous avons mis en place un graphe des dépendance pour illusatrer les trajet possible et  donc l'application peut int\'eroger le serveur pour mettre en place les différents lien possible.
